...
class TestWindow(QDialog):
    ...
    def init_ui(self):
        ...
        self.best_table.setColumnCount(5)
    ...
    def retranslate_ui(self):
        ...
        self.start_button.setText(tr('test_start_button', self.language))
        self.stop_button.setText(tr('test_stop_button', self.language))
        ...
        self.best_table.setHorizontalHeaderLabels([
            tr('best_strategies_col_rank', self.language),
    ...
    def update_best_strategies(self):
        ...
        # Сортируем стратегии по количеству успешных тестов
        # Считаем общий балл: HTTP OK + TLS1.2 OK + TLS1.3 OK + Ping OK
        sorted_strategies = []
        for strategy_name, stats in self.strategy_stats.items():
            total_score = stats['http_ok'] + stats['tls12_ok'] + stats['tls13_ok'] + stats['ping_ok']
        ...
        sorted_strategies.sort(key=lambda x: x['score'], reverse=True)
        
        # Добавляем в таблицу (топ 10 или все, если меньше)
        for idx, strategy in enumerate(sorted_strategies[:10], 1):
            row = self.best_table.rowCount()
            self.best_table.insertRow(row)
            
            # Место
            place_item = QTableWidgetItem(str(idx))
            self.best_table.setItem(row, 0, place_item)
            
            # Название стратегии
            name_item = QTableWidgetItem(strategy['name'])
            self.best_table.setItem(row, 1, name_item)
            
            # HTTP OK
            http_item = QTableWidgetItem(f"{strategy['http_ok']}/{strategy['total']}")
            self.best_table.setItem(row, 2, http_item)
            
            # TLS OK (TLS1.2 + TLS1.3)
            tls_item = QTableWidgetItem(f"{strategy['tls_ok']}/{strategy['total'] * 2}")
            self.best_table.setItem(row, 3, tls_item)
            
            # Ping OK
            ping_item = QTableWidgetItem(f"{strategy['ping_ok']}/{strategy['total']}")
            self.best_table.setItem(row, 4, ping_item)
            
            # Цветовая индикация для лучших стратегий
            if idx == 1:
                # Зеленый цвет для первого места
                for col in range(5):
                    item = self.best_table.item(row, col)
                    if item:
                        item.setForeground(QColor(0, 128, 0))  # Зеленый
            elif idx == 2:
                # Желтый цвет для второго места
                for col in range(5):
                    item = self.best_table.item(row, col)
                    if item:
                        item.setForeground(QColor(255, 255, 0))  # Желтый
            elif idx == 3:
                # Красный цвет для третьего места
                for col in range(5):
                    item = self.best_table.item(row, col)
                    if item:
                        item.setForeground(QColor(255, 0, 0))  # Красный
